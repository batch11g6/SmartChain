// https://smartchainrestapi.herokuapp.com/ 
// https://batch11smartchain.herokuapp.com/   group id
// http://127.0.0.1:8000/
/**
 * Backend Domain URL 
 */

export default {
    DOMAIN_URL: 'https://batch11smartchain.herokuapp.com/',


    VALIDITY_API_PATH: 'api/product/isvaild/',
    COUNTERFEIT_LIST_API_PATH: 'api/counterfeitlist/counterfeiltcount/',
    CITY_COUNTERFEIT_COUNT: 'api/counterfeitlist/citycount/',
    CITY_CORDS: 'api/counterfeitlist/counterfeitcords/',
}